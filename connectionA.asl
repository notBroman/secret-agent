...
cardinalDirToNum(CardinalDir,X,Y,NX,NY) :- (CardinalDir == n & NY = Y - 1 & NX = X) |
...
block_type(BlockDir, BlockType) :- cardinalDirToNum(BlockDir, 0, 0, Nx, Ny) & thing(Nx, Ny, block, BlockType).
...
+!think : lastActionResult(failed_path) & lastAction(move) <- !move_random.
+!think : not attached_block(_,_) & not my_b0(_,_) & not my_b1(_,_) <- !move_random.
+!think : attached_block(_,_) & not my_goal(X,Y) <- !move_random.
...
+!think : adjacent_thing(ListDispenser,dispenser) & not .empty(ListDispenser) & (not adjacent_thing(ListBlock,block) | .empty(ListBlock))
...
// submission logic
+!think : attached_block(CardinalDir,BType) & me(Mx, My) & my_goal(Mx, My) & task(TaskName, Due, _, Req) &
	.length(Req,Len) & Len == 1 & .nth(0,Req,R) & B == BType & not my_task(_)
	<- +my_task(TaskName); !submit_task.
+!think : attached_block(CardinalDir,BType) & me(Mx, My) & my_goal(Mx, My) & task(TaskName, Due, Reward, Req) &
	.length(Req, Len) & Len == 1 & .nth(0,Req,R) & not B == BType & not my_task(_)
	<- -task(TaskName, Due, Reward, Req); !think.
+!think : my_task(_) & lastActionResult(failed) <- rotate(cw).
+!think : my_task(_) <- !submit_task.
// fail safe
...
+!updateMyAttached : lastActionResult(success) & lastActionParams(ActionParams) & 
			lastAction(attach) & .nth(0,ActionParams,LastAction) & 
			block_type(LastAction,BType) <- +attached_block(LastAction,BType).
